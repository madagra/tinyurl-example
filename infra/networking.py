...
security_group = aws.ec2.SecurityGroup(f'{APP_NAME}-secgrp',
...
cert = acm.Certificate(f"{APP_NAME}-cert",
    domain_name=DOMAIN,
    validation_method="DNS",
)

# Route53 record for ACM valdiation
cert_validation_record = route53.Record(f"{APP_NAME}-certValidationRecord",
...
cert_validation = acm.CertificateValidation(f"{APP_NAME}-certValidation",
    certificate_arn=cert.arn,
    validation_record_fqdns=[cert_validation_record.fqdn],
)

# Create a load balancer to listen for HTTP(S) traffic
load_balancer = aws.lb.LoadBalancer(f'{APP_NAME}-lb',
	security_groups=[security_group.id],
	subnets=default_vpc_subnets.ids,
)

lb_target_group = aws.lb.TargetGroup(f'{APP_NAME}-tg',
	port=APP_PORT,
	protocol='HTTP',
	target_type='ip',
	vpc_id=default_vpc.id,
...
https_list = aws.lb.Listener(f'{APP_NAME}-https-listener',
...
http_list = aws.lb.Listener(f'{APP_NAME}-http-listener',
    load_balancer_arn=load_balancer.arn,
    port=80,
    protocol="HTTP",
    default_actions=[aws.lb.ListenerDefaultActionArgs(
...
dns_record = route53.Record(f"{APP_NAME}-dnsRecord",
    type="A",
    name=DOMAIN,
    aliases=[
        {
